  ...
  export default {
    ...
    mounted () {
      this.$nextTick(() => {
        ...
        this.infinity = this.scroll.options.infinity;
        this.fillItems();
      });
    },
    methods: {
      fillItems () {
        this.infinity.fetch(this.count).then(data => {
          for (let item of data) {
            this.$refs.chat.querySelector('ul').appendChild(this.infinity.render(item));
          }
          this.scroll.refresh();
        });
      },
